...
const EditUserModal: React.FC<EditUserModalProps> = ({
    ...
    const [selectedUserType, setSelectedUserType] = useState<'student' | 'admin' | 'seller'>(initialData.userType);
    ...
    // Helper function to check if specialty exists in predefined options
    const getSpecialtyMapping = (specialty?: string) => {
        if (!specialty) return { specialty: '', customSpecialty: '' };
        
        const found = specialtyOptions.find(opt => 
            opt.value === specialty || opt.label.toLowerCase() === specialty.toLowerCase()
        );
        
        if (found && found.value !== 'otro') {
            return { specialty: found.value, customSpecialty: '' };
        } else {
            return { specialty: 'otro', customSpecialty: specialty };
        }
    };

    // Helper function to check if hospital exists in predefined options
    const getHospitalMapping = (hospital?: string) => {
        if (!hospital) return { residencyHospital: '', customHospital: '' };
        
        const found = hospitalOptions.find(opt => 
            opt.value === hospital || opt.label.toLowerCase() === hospital.toLowerCase()
        );
        
        if (found && found.value !== 'otro') {
            return { residencyHospital: found.value, customHospital: '' };
        } else {
            return { residencyHospital: 'otro', customHospital: hospital };
        }
    };
    ...
    const resetForm = () => {
        ...
        setSelectedUserType(initialData.userType);
        setErrors({});
    ...
    useEffect(() => {
        // Process initial data to handle specialty and hospital mapping
        const specialtyMapping = getSpecialtyMapping(initialData.specialty);
        const hospitalMapping = getHospitalMapping(initialData.residencyHospital);
    ...
    const handleResidencyChange = (residency: string) => {
        ...
        setIsResidencyDropdownOpen(false);
        if (errors.residency) setErrors(prev => ({ ...prev, residency: undefined }));
    ...
    const handleHospitalChange = (hospital: string) => {
        ...
        setIsHospitalDropdownOpen(false);
        if (errors.customHospital) setErrors(prev => ({ ...prev, customHospital: undefined }));
    ...
    const validateForm = (): boolean => {
        ...
        if (selectedUserType === 'student') {
            ...
            if (!formData.licenseNumber?.trim()) newErrors.licenseNumber = 'La cédula es requerida';
            else if (formData.licenseNumber.length > 10) newErrors.licenseNumber = 'Máximo 10 caracteres';
                        ...
                        <div className="relative z-[70]">
                            ...
                            <div className="relative" ref={residencyDropdownRef}>
                                <button
                                    ...
                                    className={`w-full px-4 py-2 border rounded-lg bg-white flex items-center justify-between hover:border-purple-500 focus:outline-none transition-all duration-200 ${errors.residency ? 'border-red-500' : 'border-gray-300'
                                        }`}
                                >
                        ...
                        <div className="relative z-[65]">
                            ...
                            <div className="relative" ref={hospitalDropdownRef}>
                                <button
                                    ...
                                    className={`w-full px-4 py-2 border rounded-lg bg-white flex items-center justify-between hover:border-purple-500 focus:outline-none transition-all duration-200 ${errors.customHospital ? 'border-red-500' : 'border-gray-300'
                                        }`}
                                >
                        ...
                        <div className="relative z-[60]">
                            ...
                            <div className="relative" ref={specialtyDropdownRef}>
                                <button
                                    ...
                                    className={`w-full px-4 py-2 border rounded-lg bg-white flex items-center justify-between hover:border-purple-500 focus:outline-none transition-all duration-200 ${errors.specialty ? 'border-red-500' : 'border-gray-300'
                                        }`}
                                >
                    ...
                    <div className="mt-6">
                        <div className="flex items-start gap-3">
                            <button
                                ...
                                className={`flex-shrink-0 w-5 h-5 border-2 rounded transition-all duration-200 flex items-center justify-center ${formData.acceptPrivacyPolicy ? 'bg-purple-600 border-purple-600' : 'border-gray-300 hover:border-purple-500'
                                    } ${errors.acceptPrivacyPolicy ? 'border-red-500' : ''}`}
                            >
                                {formData.acceptPrivacyPolicy && <Check className="w-3 h-3 text-white" />}
                            </button>
                            <label
                ...
                <div className="flex-shrink-0 px-8 py-6 border-b border-gray-100">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-gray-600 text-sm mt-1">Modifica la información del usuario</p>
                        </div>
                        <button
                            onClick={handleClose}
